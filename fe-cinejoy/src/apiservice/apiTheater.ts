...
export const addTheater = async (theater: ITheater) => {
    const response = await axiosClient.post<ITheater>("/theaters", theater);
    return response.data;
};

// Cập nhật rạp
export const updateTheater = async (id: string, theater: ITheater) => {
    const response = await axiosClient.put<ITheater>(`/theaters/${id}`, theater);
    return response.data;
};

// Xóa rạp
export const deleteTheater = async (id: string) => {
    const response = await axiosClient.delete<{ message: string }>(`/theaters/${id}`);
    return response.data;
};
